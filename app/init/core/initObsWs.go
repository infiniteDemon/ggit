...
func InitObsWs() {
	...
	ObsC = Client{Host: "localhost", Port: 10086}
	if err := ObsC.Connect(); err != nil {
		global.LOG.Error("err", zap.Error(err))
		return
	}
	defer ObsC.Disconnect()

	global.LOG.Info("started polling")

	for ObsC.connected {
		m := make(map[string]interface{})
		if err := ObsC.Conn.ReadJSON(&m); err != nil {
			if !ObsC.connected {
				return
