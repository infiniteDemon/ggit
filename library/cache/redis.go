...
/**
 * @Author demon
 * @Description //TODO redis存储驱动
 * @Date 2020-7-12 20:38:21
 **/
	...
	return &RedisStore{
		pool: &redis.Pool{
			...
			Dial: func() (redis.Conn, error) {
				...
				if err != nil {
					log.Printf("无法连接redis %s 程序继续执行", err)
					return nil, err
				}
				return c, nil
			},
...
// 在列表尾部加入一个值
func (store *RedisStore) ListPush(key string, value interface{}) error {
	rc := store.pool.Get()
	defer rc.Close()
	...
	_, err = rc.Do("LPUSH", key, serialized)
