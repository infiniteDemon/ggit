...
// Init 初始化 MySQL 链接
func BackupMysql() *gorm.DB {
	dsn := fmt.Sprintf("%v:%v@(%v:%v)/%v%v",
		config.Config.Backup.User,
		config.Config.Backup.Password,
		config.Config.Backup.Host,
		config.Config.Backup.Port,
		config.Config.Backup.DbName,
		config.Config.Backup.Params,
	)
